...
- REST `/v1/search`, compatibility `/search`, and MCP `/mcp` do not implement built-in authentication.
- There is no built-in per-client rate limiting.
- Crawled content is returned as untrusted external text; downstream agents must treat it as prompt-injection-prone.
- Crawl egress is protected by orchestrator URL safety checks and the SSRF proxy, but operators remain responsible for firewalling internal networks and restricting allowed domains where needed.
