...
`POST /v1/fetch` and MCP `web_fetch` skip discovery, chunking, embedding, and reranking. They validate one to four URLs and always rerun URL safety and current robots policy before consulting the optional page cache. Cache identity preserves host and query distinctions and includes retrieval capabilities plus cleaner version. Credential-bearing URLs, raw HTML without operator opt-in, PDFs, documents, and structured extraction requests bypass persistence; the fetch contract does not accept caller-supplied headers or cookies.

The optional deterministic `links`, `tables`, and `json_ld` profiles parse a byte-bounded source-HTML prefix, exclude hidden and non-content elements, and return independent typed payloads. The 1 MiB parser prefix is a defense-in-depth ceiling beneath the configured content limit unless an operator raises that limit. JSON-LD blocks are capped at 64 KiB and limited to Article-family and Product documents. Links and table elements carry end-exclusive element spans; JSON-LD fields address the exact `<script>` element that declared them. Every retained deterministic value carries the source-document identity, cleaned-Markdown digest, full source-HTML digest, and remains untrusted page data. A failed, challenge, or source-less fetch returns an unsupported profile without a document identity; it cannot become structured evidence.

Optional `json_schema` extraction sends bounded cleaned Markdown, never raw HTML or secrets, to the configured LLM under a fixed system instruction that labels page text as untrusted data. Thorondor accepts only a bounded local schema subset, validates the returned object itself, and requires each scalar value to occur in its model-supplied exact Markdown excerpt before constructing an evidence ID. Extraction is all-or-nothing: any schema, evidence, or response-budget failure suppresses model data and returns only closed diagnostics. Schema bytes, depth, property count, URL count, prompt bytes, output bytes, leaf fields, a process-owned four-request concurrency limit, and a 20-second model deadline are enforced independently from the route limits.
...
Every candidate passes deterministic URL normalization, same-origin and seed-directory scope, include/exclude globs, query and file policy, URL safety, robots, deduplication, depth, discovery, and page limits. Requests to one host honor configured spacing, robots crawl delay, bounded jitter, parsed `Retry-After`, and adaptive 403/429 cooldown. `map` returns URL state histories and source diagnostics; `crawl` adds the same typed fetch results and optional structured profiles as known-URL fetch. The route deadline and caller cancellation cancel pending traversal rather than leaving background work running.
