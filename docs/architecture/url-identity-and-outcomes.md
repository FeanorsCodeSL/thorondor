...
The first-party Compose configurations attach Crawl4AI only to an internal network and route its target-site traffic through the SSRF proxy. The orchestrator performs non-blocking safety checks before dispatch and revalidates every changed final URL before accepting content. The identity models do not themselves authorize dispatch or bypass either enforcement layer.
